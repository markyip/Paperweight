...
- Installer: `src-tauri/target/release/bundle/nsis/Paperweight_0.1.1_x64-setup.exe` — after install, set the default in Settings → Apps → Default apps → Paperweight, or Open with. The NSIS hooks restore the `.pdf` / `.epub` extension default so Explorer thumbnails stay intact.
